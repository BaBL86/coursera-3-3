...
This script merges the training and the test sets, extracts only the measurements on the mean and standard deviation for each measurement.  
...
Open run_analysis.R file and change this path to your correct working directory:
```
setwd('/home/babl/coursera/Getting_and_Cleaning_Data/cp/coursera-3-3/')
```
with the original Human Activity Recognition Using Smartphones Dataset
files and this script.

Run run_analysis.R file with the command:
```
source('run_analysis.R')
```

Now you can read and check the data by following command:

```
data <- read.table("avg_data_set.txt")
```


## Original Human Activity Recognition Using Smartphones Dataset README file:
